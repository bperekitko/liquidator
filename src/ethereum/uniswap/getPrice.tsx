...
import { fetchSushiswapReserves, fetchUniswapReserves } from './fetchReserves';
...
export const getPriceOnUniswap = async (amount: number, input: ERC20, output: ERC20, eth: Eth): Promise<string> => {
...
export const getPriceOnSushiswap = async (amount: number, input: ERC20, output: ERC20, eth: Eth): Promise<string> => {
	const [inputReserve, outputReserve] = await fetchSushiswapReserves(input, output, eth);
	if (!inputReserve || !outputReserve) {
		return 'N/A';
	}
